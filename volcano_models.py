...
class VolcanoSimulation:
    ...
    def get_colormap(cmap_name="inferno"):
        if cmap_name == "violet_yellow":
            # FIX: reversed so yellow=low (outer) → purple=severe (inner/high intensity)
    ...
    def compute_damage_overlay(
        ...
        """
        Damage intensity field.
        - FIX: eq_mag_num now meaningfully scales output (was being divided by 9
          but never multiplied back — now properly gates the whole field so
          M0.0 = no damage, M9.0 = full damage).
        - Colormap reversed so high intensity = purple (centre), low = yellow (edge).
        ...
        # Normalise spatial shape to [0,1], then scale by both factors
        # This means: dragging eq to 0 genuinely zeroes the overlay
        peak = damage.max()
        if peak > 1e-12:
            damage /= peak
        damage = damage * scale_factor * quake_factor
    ...
    def compute_ash_overlay(
        ...
        """
        Stable Gaussian ash-plume.
        - FIX: removed hard dist_grid cutoff that was clipping the plume at
          1.5x max_radius. The Gaussian envelope now handles falloff naturally
          so the plume can extend realistically with strong winds.
        Returns (rgba_array, normalised_field).
        """
        if radius <= 0 or max_radius <= 0:
            empty = np.zeros((*self.dist_grid.shape, 4), dtype=np.uint8)
        ...
        # Logarithmic wind factor — stable across 0–200 km/h
        ...
        # Radial envelope — decays over half of max_radius
        # FIX: NO hard dist_grid cutoff here — plume fades via envelope only
        # so strong wind plumes aren't artificially clipped at the circle edge
        decay_km = max(1.0, max_radius * 0.6)
        envelope = np.exp(-self.dist_grid / decay_km)
        ash = gauss * envelope

        # Mild deterministic turbulence at edges
